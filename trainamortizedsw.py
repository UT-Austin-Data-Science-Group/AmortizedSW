...
import models
import datasets
from functions import train_amortizedsw, validate, LinearLrDecay, load_params, copy_params
...
def main():
    ...
    if(args.f_type=='linear'):
        f_slice =  Linear_Mapping(args.gen_batch_size,feat_size).cuda()
    elif(args.f_type=='glinear'):
        f_slice = Generalized_Linear_Mapping(args.gen_batch_size, feat_size).cuda()
    elif (args.f_type == 'nonlinear'):
        f_slice = Non_Linear_Mapping(args.gen_batch_size, feat_size).cuda()
    elif (args.f_type == 'max'):
        f_slice = Max(args.gen_batch_size, feat_size).cuda()
    # set optimizer
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        train_amortizedsw(args, gen_net, dis_net,f_slice, gen_optimizer, dis_optimizer,f_optimizer, gen_avg_param, train_loader, epoch, writer_dict,
              lr_schedulers)
