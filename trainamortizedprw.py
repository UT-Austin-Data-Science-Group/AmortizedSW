...
import models
import datasets
from functions import train_amortizedprw, validate, LinearLrDecay, load_params, copy_params
...
from amortized_functions import *
np.random.seed(1)
torch.manual_seed(1)
...
def main():
    ...
    if(args.dataset.lower() == 'stl10'):
        ...
        feat_size=8193
    if(args.f_type=='linear'):
        f_slice =  Linear_Mapping_K(args.gen_batch_size,feat_size,args.k).cuda()
    elif(args.f_type=='glinear'):
        f_slice = Generalized_Linear_Mapping_K(args.gen_batch_size, feat_size,args.k).cuda()
    elif (args.f_type == 'nonlinear'):
        f_slice = Non_Linear_Mapping_K(args.gen_batch_size, feat_size,args.k).cuda()
    elif (args.f_type == 'max'):
        f_slice = Max_K(args.gen_batch_size, feat_size,args.k).cuda()
    # set optimizer
    gen_optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, gen_net.parameters()),
                                     args.g_lr, (args.beta1, args.beta2))
    dis_optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, dis_net.parameters()),
                                     args.d_lr, (args.beta1, args.beta2))
    f_optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, f_slice.parameters()),
                                     args.s_lr, (args.beta1, args.beta2))
    
    gen_scheduler = LinearLrDecay(gen_optimizer, args.g_lr, 0.0, 0, args.max_iter * args.n_critic)
    dis_scheduler = LinearLrDecay(dis_optimizer, args.d_lr, 0.0, 0, args.max_iter * args.n_critic)
    f_scheduler = LinearLrDecay(f_optimizer, args.s_lr, 0.0, 0, args.max_iter * args.n_critic)
    # set up data_loader
    ...
    # initial
    if(args.dataset.lower() == 'lsun_church' or 'celebahq'):
    ...
    iscores=[]
    fids=[]
    lr_schedulers = (gen_scheduler, dis_scheduler,f_scheduler) if args.lr_decay else None
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        train_amortizedprw(args, gen_net, dis_net,f_slice, gen_optimizer, dis_optimizer,f_optimizer, gen_avg_param, train_loader, epoch, writer_dict,
              lr_schedulers)
        ...
        save_checkpoint({
            ...
            'f_state_dict': f_slice.state_dict(),
