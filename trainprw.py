...
import models
import datasets
from functions import train_prw, validate, LinearLrDecay, load_params, copy_params
...
def main():
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        train_prw(args, gen_net, dis_net, gen_optimizer, dis_optimizer, gen_avg_param, train_loader, epoch, writer_dict,
              lr_schedulers)
